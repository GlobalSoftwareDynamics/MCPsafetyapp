...
$datos=NumOcurrenciasAnio($_POST['anio'],$link);
$datos1=NumOcurrenciasAnioxPlantaComparacion($_POST['anio'],$link);
$labels1=multilabel('Planta',$link);
$datos2=NumClaseOCxAnio($_POST['anio'],$link);
$labels2=multilabelconid('Clase','categoria','OC',$link);
$datos3=NumAccionesCorrectivasxAnio($_POST['anio'],$link);
$labels3=multilabel('EstadoACMS',$link);
?>
...
// Instantiate and draw our chart, passing in some options.
    ...
    function drawChart1() {
        ...
        var options = {
            title: "Comparación del Número de OC Reportadas por Planta",
            titleTextStyle: {fontSize: 18, bold: true},
            width: '100%',
            height: 300,
            legend: { position: 'top'}
...
<section class="container-fluid">
    <div class="col-md-6 col-md-offset-3">
        <h3 class="text-center">Reporte Anual para Todas las Plantas (OC)</h3>
    </div>
</section>
<hr>
...
<section class="container">
    ...
    <table class="table table-bordered text-center">
        ...
        while ($fila0=mysqli_fetch_array($result0)){
            $result1=mysqli_query($link,"SELECT * FROM MEOCUR WHERE idMejoras ='".$fila0['idMejoras']."' AND idOcurrencias IN (SELECT idOcurrencias FROM Ocurrencias WHERE estado ='Aprobado' AND idOcurrencias LIKE 'OC".$año[1]."%')");
...
<section class="container">
    ...
    <table class="table table-bordered text-center">
        ...
        while ($fila0=mysqli_fetch_array($result0)){
            $result1=mysqli_query($link,"SELECT * FROM ACOCUR WHERE idAccionesCorrectivas ='".$fila0['idAccionesCorrectivas']."' AND idOcurrencias IN (SELECT idOcurrencias FROM Ocurrencias WHERE estado ='Aprobado' AND idOcurrencias LIKE 'OC".$año[1]."%')");
            while ($fila1=mysqli_fetch_array($result1)){
                echo "
                            <td>".$fila0['fecharegistro']."</td>
                ";
